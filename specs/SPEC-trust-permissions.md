...
Orchestration is unavailable until Pi reports the project trusted. The extension rejects an untrusted project before invoking any operating-system probe, reading project orchestration configuration, or creating journals, workspaces, transports, panes, or agents. In a trusted TUI, `/subagents doctor` may perform the same read-only runtime/repository probes without task text, an active model, or model authentication; untrusted doctor requests report the trust requirement without probing. After trust, run preflight additionally requires TUI mode, non-empty task text, and configured authentication for the active parent model before inspecting repository or external-tool facts. Direct-task admission then requires explicit safe declared write paths, a clean conflict-free source working copy, and no unfinished retained run; it creates no resources and does not recover retained work. The standalone `pi-subagents doctor --json` command diagnoses only externally observable runtime/repository capability from its explicitly selected current directory; it cannot inspect Pi session trust or model authentication and creates no resources.
