...
The publication service revalidates mutable artifact/repository facts, journals transport intent before mutation and outcome afterward, and delegates one exact revision to the coordinator-only local Git transport adapter. That adapter owns a bare `transport.git` inside the coordinator state directory, creates only `pi-subagents/<run>/<task>/<attempt>` refs, fetches before Jujutsu's lease-protected push, directly verifies the bare ref, fetches that one ref into the coordinator repository, and verifies commit/change/base identities again. No decomposer, DAG, multi-worker scheduler, reviewer launch, integration adapter, or transport cleanup exists yet.
