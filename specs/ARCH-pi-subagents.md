...
The reviewer supervisor first verifies the coordinator's fetched ref and assigned base, then creates a separate exact Rift snapshot from that coordinator repository rather than the worker/source workspace. Its reviewer guard exposes a fixed base-to-reviewed-commit diff only. After settlement it rechecks the result, review target, source ref/base, and unchanged reviewer snapshot working-copy identity before journaling `review_approved` or `review_revision_requested`. No decomposer, DAG, multi-worker scheduler, revision loop, integration adapter, or transport cleanup exists yet.
