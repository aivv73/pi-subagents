...
The shared doctor service produces a strict version-one report of those runtime/repository checks, each check's passed/failed status, bounded safe evidence, and coordinator-authored remediation. In a trusted Pi TUI, `/subagents doctor` renders a concise human summary; `pi-subagents doctor --json` writes the same report as its sole stdout record, exits zero only when all required checks pass, exits one for an unsupported environment, and exits two for invalid invocation or unexpected diagnostic failure. Both paths are read-only and create no orchestration resources. The standalone command intentionally cannot assess Pi session trust or active-model authentication.
