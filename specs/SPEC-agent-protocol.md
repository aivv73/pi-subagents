...
The result-writing tool describes the exact coordinator-owned run/task/attempt fields the child must echo; its worker template leaves only the current Jujutsu identities and changed declared paths for the worker to supply, while its reviewer template fixes the reviewed commit and base. This exposes no artifact path or general configuration-reading capability and does not make envelope files readable to the child.
